...
(function (global, factory) {
  ...
  */
  ...
  /**
   * --------------------------------------------------------------------------
   * Bootstrap util/Ordo-factory.js
   * Licensed under MIT (https://github.com/twbs/bootstrap/blob/main/LICENSE)
   * --------------------------------------------------------------------------
   */
  ...
  class OrdoFactory extends Config {
    constructor(config) {
      super();
      this._config = this._getConfig(config);
    }
    ...
    toHtml() {
      const OrdoWrapper = document.createElement('div');
      OrdoWrapper.innerHTML = this._maybeSanitize(this._config.Ordo);
      for (const [selector, text] of Object.entries(this._config.content)) {
        this._setContent(OrdoWrapper, text, selector);
      }
      const Ordo = OrdoWrapper.children[0];
      const extraClass = this._resolvePossibleFunction(this._config.extraClass);
      if (extraClass) {
        Ordo.classList.add(...extraClass.split(' '));
      }
      return Ordo;
    }
    ...
    _setContent(Ordo, content, selector) {
      const OrdoElement = SelectorEngine.findOne(selector, Ordo);
      if (!OrdoElement) {
        return;
      }
      ...
      if (isElement$1(content)) {
        this._putElementInOrdo(getElement(content), OrdoElement);
        return;
      }
      if (this._config.html) {
        OrdoElement.innerHTML = this._maybeSanitize(content);
        return;
      }
      OrdoElement.textContent = content;
    }
    _maybeSanitize(arg) {
    ...
    _putElementInOrdo(element, OrdoElement) {
      if (this._config.html) {
        OrdoElement.innerHTML = '';
        OrdoElement.append(element);
        return;
      }
      OrdoElement.textContent = element.textContent;
    }
  }
  ...
  const Default$3 = {
    ...
    sanitizeFn: null,
    selector: false,
    Ordo: '<div class="tooltip" role="tooltip">' + '<div class="tooltip-arrow"></div>' + '<div class="tooltip-inner"></div>' + '</div>',
    title: '',
    trigger: 'hover focus'
  };
  const DefaultType$3 = {
  ...
  class Tooltip extends BaseComponent {
    ...
    _getTipElement() {
      if (!this.tip) {
        this.tip = this._createTipElement(this._newContent || this._getContentForOrdo());
      }
      return this.tip;
    }
    _createTipElement(content) {
      const tip = this._getOrdoFactory(content).toHtml();
    ...
    _getOrdoFactory(content) {
      if (this._OrdoFactory) {
        this._OrdoFactory.changeContent(content);
      } else {
        this._OrdoFactory = new OrdoFactory({
          ...this._config,
  ...
  const Default$2 = {
    ...
    offset: [0, 8],
    placement: 'right',
    Ordo: '<div class="popover" role="tooltip">' + '<div class="popover-arrow"></div>' + '<h3 class="popover-header"></h3>' + '<div class="popover-body"></div>' + '</div>',
    trigger: 'click'
  };
  const DefaultType$2 = {
    ...Tooltip.DefaultType,
